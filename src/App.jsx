...
const socket = io("https://queue-cure-production-b559.up.railway.app");
...
function App() {
  ...
  const addPatient = () => {
    ...
    setPatients(updated);
    socket.emit("queueUpdate", { patients: updated, currentToken, servedCount, avgTime });
    setToken(token + 1);
    setPatientName("");
  };
  ...
  const callNext = () => {
    ...
    const utterance = new SpeechSynthesisUtterance(
      `Attention please. Token number ${next.tokenNo}, ${next.name}, please proceed to ${next.doctor}`
    );
    utterance.rate = 0.85;
    utterance.pitch = 1;
    utterance.volume = 1;
    ...
    socket.emit("queueUpdate", { patients: updated, currentToken: next, servedCount, avgTime });
  };
  ...
  const completeConsultation = () => {
    ...
    socket.emit("queueUpdate", { patients, currentToken: null, servedCount: count, avgTime });
  };
  ...
  return (
    <div className="dashboard">
      ...
      <header className="hero">
        <h1>🏥 Queue Cure '26</h1>
        <p>Real-Time Digital Queue Management for Clinics</p>
        <p className="hero-sub">
          Receptionist assigns tokens in under 10 seconds. Patients receive live queue updates instantly.
        </p>
        <button className="done" onClick={() => setLoggedIn(false)}>
          Logout
        </button>
      </header>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          <h3>Now Serving</h3>
          <h2>{currentToken ? currentToken.tokenNo : "--"}</h2>
        </div>
        <div className="stat-card">
          <h3>Emergency Cases</h3>
          <h2>{patients.filter(p => p.priority === "Emergency").length}</h2>
        </div>
      </div>
      ...
      <div className="main-grid">
        ...
        <div className="panel">
          ...
          <div className="button-grid">
            <button className="add" onClick={addPatient}>Add Patient</button>
            <button className="next" onClick={callNext}>Call Next</button>
            <button className="skip" onClick={skipPatient}>Skip</button>
            <button className="done" onClick={completeConsultation}>Complete</button>
          </div>

          <div className="avg-box">
            <label>Consultation Time (mins)</label>
            <input
              type="number"
              value={avgTime}
      ...
      <div className="waiting-room">
        ...
        <div className="waiting-stats">
          <div>
            <h4>Current Token</h4>
            <h1>{currentToken ? currentToken.tokenNo : "--"}</h1>
          </div>
          <div>
          ...
          <div>
            ...
            <h1>{patients.length * avgTime} min</h1>
          </div>
        </div>
      </div>
      ...
      <div className="panel appointment-panel">
        ...
        />
        <br /><br />
        <button className="add" onClick={bookAppointment}>
          Book Appointment
        </button>
        <br /><br />
